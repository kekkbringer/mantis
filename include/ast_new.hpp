...
namespace ast_new {
     ...
     */
    struct Decl {
        enum class Kind {Var, Func} kind; ///< kind of the declaration
     ...
     */
    struct For_init {
        enum class Kind {None, Decl, Expr} kind = Kind::None; ///< kind of the initialization clause
        union {
            Decl* decl; ///< pointer to the init, in this case a declaration
            Expr* expr; ///< pointer to the init, in this case an expression
        };
        For_init() : kind(Kind::None), decl(nullptr) {}
        explicit For_init(Decl* d) : kind(Kind::Decl), decl(d) {}
