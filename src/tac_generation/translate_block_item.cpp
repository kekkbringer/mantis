...
void Tac_generator::translate_block_item(const ast::Block_item* bitem, std::vector<tac::Inst*>& insts) {
        ...
        if (bitem->decl->kind == ast::Decl::Kind::Var) {
            ...
            if (var_decl->init != nullptr) {
                ...
                if (var_decl->storage_class != ast::Decl::Storage_class::Static) {
                    const auto init_val = translate_expression(var_decl->init, insts);
                    auto copy_ptr = arena->allocate<tac::Copy>(tac::Copy(init_val, tac::Value::make_variable(var_decl->name)));
                    insts.emplace_back(copy_ptr);
                }
            }
        }
    }
